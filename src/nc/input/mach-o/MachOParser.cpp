...
private:
    ...
    void parseDySymtabCommand() {
        ...
        log_.debug(tr("Found a symbol table with %1 local entries.").arg(command.nlocalsym));
        log_.debug(tr("Found a symbol table with %1 extdef entries.").arg(command.nextdefsym));
        log_.debug(tr("Found a symbol table with %1 undef entries.").arg(command.nundefsym));
        ...
        foreach (const auto &indirectSection, indirectSections_) {
            ...
            for (uint32_t j = 0; j < indirectSection.size; j += indirectSection.stride) {
                using core::image::Symbol;
                using core::image::SymbolType;
                ...
                if (! (symbolIndex & INDIRECT_SYMBOL_LOCAL || symbolIndex & INDIRECT_SYMBOL_ABS)) {
                    ...
                    image_->addSymbol(std::make_unique<core::image::Symbol>(sym->type(), sym->name(), indirectSection.addr + j, indirectSection.section));
                }
            }
        }
    }
